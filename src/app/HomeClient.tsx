...
import { arrowLeftIcon, arrowRightIcon } from '@/components/uiIconPaths';
...
import WateryPoints from '@/components/WateryPoints';
import ButterflyMotif from '@/components/ButterflyMotif';
import { HeroMotion, CraftInterlude, CraftCursorTrail } from '@/components/CraftMotion';
...
export default function HomeClient({ products, settings }: { products: Product[]; settings: SiteSettings }) {
  ...
  useEffect(() => {
    ...
    homeRef.current?.querySelectorAll('section').forEach(section => observer.observe(section));
    return () => observer.disconnect();
  }, []);
  const [productStart, setProductStart] = useState(0);
  const { stats: STATS, marqueeItems: MARQUEE_ITEMS } = settings;
  const featuredProducts = products.length ? Array.from({ length: Math.min(3, products.length) }, (_, offset) => products[(productStart + offset) % products.length]) : [];
  return (
    <MotionConfig reducedMotion={paused ? 'always' : 'user'}><div ref={homeRef} className={`home-page motion-rich bg-cream ${paused ? 'home-motion-paused' : ''}`}>
      <CraftCursorTrail paused={paused} />
      {/* Hero Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-8">
        <div className="home-hero heritage-hero relative bg-navy rounded-[2.5rem] overflow-hidden flex flex-col md:flex-row items-center">
          <WateryPoints count={12} />
          <HeroMotion paused={paused} />
          ...
          <div className="hero-copy relative z-10 px-6 md:px-8 py-10 text-cream max-w-2xl">
            <SectionLabel index="00" className="text-gold mb-6">{settings.heroEyebrow}</SectionLabel>
            <RevealText
              as="h1"
              lines={settings.heroLines}
              delay={0.25}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="mt-8">
              <div className="flex flex-wrap items-center gap-6"><Magnetic><Link href="/products" className="home-shop-link shimmer">Explore the collection <span aria-hidden="true">↗</span></Link></Magnetic>
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 1 }}
              className="hero-scroll-cue text-cream/70 mt-12"
            >
              <i aria-hidden="true" /> Scroll to explore
            </motion.div>
          </div>
      ...
      <div className="heritage-ribbon bg-navy text-gold py-5 md:py-7 border-y border-gold/25 my-4">
        <Marquee items={MARQUEE_ITEMS} speed={20} size="display" />
      </div>

      {/* Featured Products */}
      <section className="featured-relics max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
        <DriftMotif kind="diamond" className="w-16 h-16 right-2 top-4 text-gold hidden lg:block" />
        <div className="home-section-heading"><div><SectionLabel index="01" className="text-maroon mb-3">Objects with a story</SectionLabel><RevealText lines={[settings.featuredHeading]} className="section-display text-navy-dark" /></div><Link href="/products" className="home-text-link text-maroon">View all pieces <span aria-hidden="true">↗</span></Link></div>
        <div className="flex items-center gap-6 md:gap-10">
          <button aria-label="Previous featured products" disabled={products.length <= 3} onClick={() => setProductStart(i => ((Math.floor(i / 3) - 1 + Math.ceil(products.length / 3)) % Math.ceil(products.length / 3)) * 3)} className="hidden md:flex home-carousel-button text-navy hover:text-maroon transition-colors flex-shrink-0 disabled:opacity-30">
            <UiIconSvg icon={arrowLeftIcon} size={36} />
          </button>

          <motion.div
            className="featured-relic-grid flex-1"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
          ...
          >
            {featuredProducts.map((product) => (
              <motion.div
                key={product.id}
                variants={{ hidden: { opacity: 0, y: 70, rotate: -4 }, visible: { opacity: 1, y: 0, rotate: 0, transition: { type: 'spring', stiffness: 100, damping: 18 } } }}
              >
                <ProductCard product={product} paused={paused} sizes="(max-width: 639px) 92vw, (max-width: 1023px) 44vw, 350px" />
              </motion.div>
            ))}
          </motion.div>

          <button aria-label="Next featured products" disabled={products.length <= 3} onClick={() => setProductStart(i => ((Math.floor(i / 3) + 1) % Math.ceil(products.length / 3)) * 3)} className="hidden md:flex home-carousel-button text-navy hover:text-maroon transition-colors flex-shrink-0 disabled:opacity-30">
            <UiIconSvg icon={arrowRightIcon} size={36} />
          </button>
        </div>
        {products.length > 3 && <div className="flex justify-center gap-3 mt-7" aria-label="Featured product groups">{Array.from({length: Math.ceil(products.length / 3)}, (_, i) => i * 3).map(start => <button key={start} aria-label={`Show featured products from ${start + 1}`} aria-pressed={productStart === start} onClick={() => setProductStart(start)} className="product-page-dot"><span className={productStart === start ? 'bg-maroon' : 'bg-navy/25'} /></button>)}</div>}
      </section>

      <CraftInterlude paused={paused} />
      ...
      <section className="home-craft-section max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <motion.div
            initial={{ opacity: 0, x: -60, rotate: -6, scale: .9 }}
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="bg-gold rounded-3xl p-10 pt-8 shimmer" style={{ ["--shimmer-delay" as string]: "1.2s" }}>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="home-section-heading"><div><SectionLabel index="04" className="text-maroon mb-3">A little love, from your homes</SectionLabel><RevealText lines={[settings.testimonialsHeading]} className="section-display text-navy-dark" /></div><ButterflyMotif className="w-16 h-16 text-maroon hidden sm:block" size={64} /></div>
